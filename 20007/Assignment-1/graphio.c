...
Graph load_graph(char *input) 
{
    ...
    Graph graph; /* Pointer to a graph of type graph_t */
...
static void initialize_vertex(Graph graph, int id)
 {
    ...
    assert(graph->vertices[id].label);
...
void print_graph(char *output, Graph graph) 
{
    ...
    for(i=0; i<graph->order; i++)
    {
        ...
        /* I don't use char[] to store the formatting since we
         * use it only once
         */
        /* Formatting */
        fprintf(output_file, "\n");
 ...
 *
 * Input Parameters : file that will be used to load vertex
