...
/* #define MAX_TRY_PERLEN 10000000 - This was part of the
implementation in the brackets */                    
...
void collide_dumb(unsigned int size, unsigned int seed, int n) 
{
	...
	str[len - 1] = ASCII_SMALLEST_PRINTABLE;
	...
	while(count < n)
	{
		...
		{
			str[len - 1] = ASCII_SMALLEST_PRINTABLE + i;

			for(j=0; j<=(ASCII_LARGEST_PRINTABLE - ASCII_SMALLEST_PRINTABLE); j++)
			{
				str[len] = ASCII_SMALLEST_PRINTABLE + j; 
				str[len + 1] = '\0';
		...
		if(len == MAXSTRLEN - 1)
		{
			fprintf(stderr, "Can't generate more strings for collision !! Sorry !!\n");
			exit(EXIT_FAILURE);
		}
		len++;
	}
...
/* 
 * Function takes the expected size of the hash table as input and
 * the generate a size which is the lowest prime number greater 
 * than twice the size of the hash table
 *
 * Input Parameter : the expected number of inputs to the hash
 * function, unsigned int n
...
void collide_clever(unsigned int size, unsigned int seed, int n) 
{
	...
	while(count < n)
	{
		...
		{
			printf("Cannot generate more strings !! Sorry !!");
			exit(1);
		}
	}
...
void collide_dumb(unsigned int size, unsigned int seed, int n) 
{
	...
	unsigned char *check_str = (unsigned char *)malloc(sizeof(unsigned char)*(MAXSTRLEN + 1));
	while(count < n)
	{
		...
		if(universal_hash(check_str, size) == 0)
		{
			...
			if(length > MAXSTRLEN)
			{
				printf("Can't generate more strings !! Sorry !!");
				exit(1);
			}
		} 
