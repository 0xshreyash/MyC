 ...
 *
 * This module contains functions useful for related to the Topological 
 * Sorting of a graph. 
 *
...
/* Returns a list with very vertex marked white i.e. unvisited */
...
List dfs_sort(Graph graph) 
{
	 ...
	 * 
	 * I used a char array instead of two arrays of integers in 
	 * order to save memory and because it extends the concept of
	 * colouring vertices described in lectures and discussed in
	 * Inroduction to Algorithms, CLRS
	 *
	 * We mark every vertex as being unvisited and initialize the
	 * graph to be white i.e. unvisited
	 */
	...
	{
		/* Call dfs sort only if the vertex has not been completed
		 * yet
		 */
		if(visited[i] !=  'b')
		{
...
static void dfs_visit(Graph graph, char *visited, List *sorted, int *id)
{
	...
	if(visited[*id] == 'w')
	{
		...
		/* Add the current vertex to the head of list of sorted lists
		 * when we are done with adding vertex that is a child 
		 * (depends on the current vertex) into the list of of sorted
		 * vertices
		 */ 
...
{
	/* current vertex stores the id of the vertex that will be
	 * taken from the list of vertices that have no incoming
	 * edges 
	 */
	int *current_vertex;
	...
	while(no_incoming != NULL)
	{
		...
		/* adding the popped vertex to the list of sorted
		 * elements at the tail from the 
		 * algorithm on wikipedia would be 
		 *
 ...
 *
 * Returned Value : A list of edges with no incoming edges
 */
 ...
 * List in for each vertex, we can just decrease the length of 
 * the list by one each time we want to delete an edge instead of
 * actually having to delete the edge which may potentially take 
 * O(V) time.
 *
...
bool verify(Graph graph, List vertices) 
{
	...
	/* Now since we have knowledge of position of each 
	 * vertex in the list it is easy for us to verify 
	 * if the list given is a topological sort of the 
	 * graph since, every vertex that a vertex A has
	 * an outgoing edge to should have a position 
	 * greater than A i.e. it's value in the 
	 * vertex_position_array must be more than the 
	 * value of A
	 */
	while(vertices != NULL)
	{
		 ...
		 * a valid topological ordering since if Vertex B 
		 * depends on Vertex A then it must lie after A 
